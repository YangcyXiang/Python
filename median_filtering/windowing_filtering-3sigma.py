...
def median_filtering(image, kernel_size):
    file_output = open(r"timoxi_640x512_grayscale_python_output-3sigma.txt", 'w')
    ...
    for row in range(image_rows):  # 512
        ...
        for col in range(image_cols):  # 640
            ...
            # significance0 = bool(np.int16(abs(fifo[image_cols + 2 + 1] - fifo[0]) > threshold))
            # significance1 = bool(np.int16(abs(fifo[image_cols + 2 + 1] - fifo[1]) > threshold))
            # significance2 = bool(np.int16(abs(fifo[image_cols + 2 + 1] - fifo[2]) > threshold))
            # significance3 = bool(np.int16(abs(fifo[image_cols + 2 + 1] - fifo[image_cols + 2])) > threshold)
            # significance5 = bool(np.int16(abs(fifo[image_cols + 2 + 1] - fifo[image_cols + 2 + 2])) > threshold)
            # significance6 = bool(np.int16(abs(fifo[image_cols + 2 + 1] - fifo[2 * (image_cols + 2)])) > threshold)
            # significance7 = bool(np.int16(abs(fifo[image_cols + 2 + 1] - fifo[2 * (image_cols + 2) + 1])) > threshold)
            # significance8 = bool(np.int16(abs(fifo[image_cols + 2 + 1] - fifo[2 * (image_cols + 2) + 2])) > threshold)
            # counter = 0
            # if significance0:
            #     counter += 1
            # if significance1:
            ...
            #     counter += 1
            # if counter > 3:
            #     image_result[row, col] = temp   # filtering
            # else:
            #     image_result[row, col] = fifo[image_cols + 2 + 1]   # raw pixel
...
cv2.imwrite("timoxi_640x512_grayscale_windowing_filter-3sigma.jpg", img_filtered)
