...
re_pattern = re.compile("in\[0\]=(\s)*(?P<in0>\d+), in\[1\]=(\s)*(?P<in1>\d+), in\[2\]=(\s)*(?P<in2>\d+), in\[3\]=(\s)*(?P<in3>\d+), in\[4\]=(\s)*(?P<in4>\d+), in\[5\]=(\s)*(?P<in5>\d+), in\[6\]=(\s)*(?P<in6>\d+), in\[7\]=(\s)*(?P<in7>\d+), in\[8\]=(\s)*(?P<in8>\d+), med=(\s)*(?P<med>\d+), is_blind=(\s)*(?P<is_blind>\d+)")
line = "in[0]=   0, in[1]=   0, in[2]=   0, in[3]=   0, in[4]=   0, in[5]=   0, in[6]=   0, in[7]=   0, in[8]=   0, med=   0, is_blind=1"
# re_result = re_pattern.search(line)
# print(re_result)

input_file_name = r"../tb_windowing_output.txt"
...
while True:
    ...
    if re_result is None:
        ...
        is_blind = int(re_result.group('is_blind'))
